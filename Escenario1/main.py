...
for epoca in range(EPOCAS):
    ...
    for i in range(0, X_train.shape[0], TAMAÑO_BATCH):
        ...
        # (Opcional) Calculamos precisión al final de cada época para ver si aprende
        # Hacemos un forward rápido con todo el set de prueba
        test_preds, _ = propagacion_adelante(X_test, W1, b1, W2, b2)
        # argmax nos dice cuál es la neurona con número más alto (la predicción)
        predictions = np.argmax(test_preds, axis=1)
        accuracy = np.mean(predictions == y_test)
        print(f"Época {epoca+1}/{EPOCAS} - Precisión en Test: {accuracy:.4f}")
